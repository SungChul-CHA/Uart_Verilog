    ...
    always @ (*) begin
        if (PARITY_BIT == 0)
            case (c_state)
                ...
                DATA_ST: if(data_finish) n_state = STOP_ST; else n_state = DATA_ST;
                STOP_ST: if(stop_finish) n_state = IDLE_ST; else n_state = STOP_ST;
            ...
            case (c_state)
                IDLE_ST: if(uart_start_pulse) n_state = START_ST; else n_state = IDLE_ST;
                START_ST: if(clk_b) n_state = DATA_ST; else n_state = START_ST;
                DATA_ST: if(data_finish) n_state = PARITY_ST; else n_state = DATA_ST;
                PARITY_ST: if(clk_b) n_state = STOP_ST; else n_state = PARITY_ST;
                STOP_ST: if(stop_finish) n_state = IDLE_ST; else n_state = STOP_ST;
                default: n_state = IDLE_ST;
            endcase
    end
